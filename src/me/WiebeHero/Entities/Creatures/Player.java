...
public class Player extends Creature{
	...
	public void tick() {
		...
		this.getInput();
		this.move();
		Game.handler.getGameCamera().centerOnEntity(this);
		//Attack
		this.checkAttacks();
	}
	...
	private void checkAttacks() {
		...
		if(Game.handler.getKeyManager().aUp) {
			ar.x = cb.x + cb.width / 2 - arSize / 2;
			ar.y = cb.y - arSize;
		}
		else if(Game.handler.getKeyManager().aDown) {
			ar.x = cb.x + cb.width / 2 - arSize / 2;
			ar.y = cb.y + cb.height;
		}
		else if(Game.handler.getKeyManager().aLeft) {
			ar.x = cb.x - arSize;
			ar.y = cb.y + cb.height / 2 - arSize / 2;
		}
		else if(Game.handler.getKeyManager().aRight) {
			ar.x = cb.x + cb.width;
			ar.y = cb.y + cb.height / 2 - arSize / 2;
		}
		else {
	...
	public void render(Graphics g) {
		g.drawImage(this.getCurrentAnimationFrame(), (int)(this.x - Game.handler.getGameCamera().getxOffset()), (int)(this.y - Game.handler.getGameCamera().getyOffset()), this.width, this.height, null);
		g.drawImage(this.slashExample.getCurrentFrame(), (int)(attackX - Game.handler.getGameCamera().getxOffset()), (int)(attackY - Game.handler.getGameCamera().getyOffset()), 32, 32, null);
